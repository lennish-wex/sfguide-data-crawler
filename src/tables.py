...
def get_table_comment(tablename, session):
    ...
    return session.sql(f"SHOW TABLES LIKE '{tbl}' IN SCHEMA {schema} LIMIT 1")\
        .collect()[0]['comment']\
        .replace("'", "\\'")
...
def convert_vec2array(tablename, session):
    ...
    vec_cols = [c.name for c in df.schema.fields if isinstance(type(c.datatype), T.VectorType)]

    if vec_cols:
        return df.select([F.array_slice(F.to_array(x), F.lit(0), F.lit(10)).as_(x)
                            if x in vec_cols else x for x in df.columns])
...
def sample_tbl(tablename, sampling_mode, n, session):
    ...
    if sampling_mode == "fast": # Randomly sample
        ...
        samples = df.withColumn('ROWNUMBER',
                            F.row_number().over(Window.partition_by()\
                                                      .order_by(F.desc(
                                                          F.call_udf('PCTG_NONNULL',
                                                                     F.array_construct('*'))))))\
                .sort(F.col('ROWNUMBER')).drop(F.col('ROW_NUMBER'))\
                .select(F.to_varchar(F.array_slice(F.array_agg(F.object_construct('*')),
                                                   F.lit(0),
                                                   F.lit(n))))\
                .to_pandas().values[0][0]
    else:
...
def run_complete(session, tablename, model, sampling_mode, n, prompt, temperature = None):
    ...
    try:
        ...
        if 'max tokens' in str(e):
            raise NotImplementedError(f"{e}.\nCortex token counter will be added once available. \
Try a different model or fewer sample rows.") from e
...
def get_crawlable_tbls(session,
    ...
    if ignore_catalog:
        ...
        catalog_constraint = f"""
NATURAL FULL OUTER JOIN {catalog_database}.{catalog_schema}.{catalog_table}
...
C AS (
    SELECT
        TABLE_SCHEMA
        ,TABLE_CATALOG || '.' || TABLE_SCHEMA || '.' || TABLE_NAME AS TABLENAME
        ,LISTAGG(CONCAT(COLUMN_NAME,
                        ' ',
                        DATA_TYPE,
                        COALESCE(concat(' (', REGEXP_REPLACE(COMMENT, '{{|}}',''), ')'), '')),
                ', ') as COLUMN_INFO
...
COALESCE(TABLE_COMMENT, 'No comment') || ', Columns: ' || COLUMN_INFO AS TABLE_DDL
FROM T NATURAL INNER JOIN C
"""
    # F-string interpolation later so remove unexpected curly
    # brackets here in data such as in comments
    ...
    """ Catalogs table objects in Snowflake.
        Args:
            ...
            sampling_mode (string): How to retrieve sample data records for table.
                                    ...
                                    - Pass 'fast' or omit to randomly sample records
                                      from each table.
                                    - Pass 'nonnull' to prioritize least null records
                                      for table samples.
